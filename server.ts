...
async function startServer() {
  ...
  const syncCrmData = async () => {
    try {
      ...
      console.log(`[INFO] CRM Data Synced: ${orders.length} orders, ${promotions.length} promotions, ${customers.length} customers`);
    } catch (e) {
      console.error("[ERROR] CRM Sync Failed:", e);
    }
  };
  ...
  app.post("/api/webhook", async (req, res, next) => {
    try {
      ...
      console.log(`[INFO] Webhook received message from ${sender}: ${message.substring(0, 50)}...`);

      // --- CRM Integration: Recognize User ---
      const existingCustomer = customers.find(c => c.phone === sender || c.email === sender);
      if (existingCustomer) {
        console.log(`[INFO] Recognized Customer: ${existingCustomer.name} (${sender})`);
      }
        ...
        try {
          ...
          if (updateData.email) {
            await googleSheets.updateCustomer({
              ...updateData,
              phone: updateData.phone || sender
            });
            await syncCrmData();
            botResponse.reply += "\n\n✅ *Profile Updated*: Your information has been synchronized with our CRM.";
            console.log(`[INFO] CRM updated for customer: ${updateData.email}`);
          }
        } catch (crmError) {
          console.error("[ERROR] CRM Update Failed:", crmError);
          botResponse.reply += "\n\n⚠️ *CRM Sync Error*: We couldn't update your profile at this moment.";
        }
      }
      ...
      if (botResponse.intent === "TRACKING" || botResponse.intent === "DELIVERY_STATUS") {
        ...
        if (orderIdMatch) {
          ...
          if (order) {
            const updatesText = order.updates.map((u: any) => `• *${u.date}*: ${u.status} (${u.location})`).join('\n');
